{*************************************************************************}
{ TISFEdit & TISFMaskEdit component                                       }
...
uses
  Windows, Registry, Dialogs,  Messages, SysUtils, Classes, Graphics,
  Controls, Forms, StdCtrls, Clipbrd, Mask, Consts, Inifiles;
...
{$ENDIF}
  ...
  published
    ...
    property LabelPosition: TLabelPosition read FLabelPosition write SetLabelPosition default lpTopLeft;
    property LabelMargin: Integer read FLabelMargin write SetLabelMargin default 2;
...
function TISFEdit.EStrToFloat(s: string): extended;
begin
...
{$ENDIF}
{$IFDEF TMSDOTNET}
  s[Length(s) - FPrecision] := FormatSettings.decimalseparator[1];
{$ENDIF}
  try
    Result := StrToFloat(s);
  except
...
function TISFEdit.CharFromPos(pt: TPoint): Integer;
begin
  Result := Loword(SendMessage(self.Handle, EM_CHARFROMPOS, 0, makelparam(pt.x, pt.y)));
end;

function TISFEdit.PosFromChar(uChar: word): TPoint;
var
  pt: tpoint;
  l: Integer;
  DC: HDC;
...
function TISFEdit.GetTextSize: Integer;
var
  DC: HDC;
  sz: TSize;
  holdFont: THandle;
...
procedure TISFEdit.InvalidateCaret(pt: TPoint);
var
  r: TRect;
begin
...
procedure TISFEdit.DrawCaretByCursor;
var
  nChar: Integer;
  pt, ptCursor: TPoint;
begin
...
procedure TISFEdit.SetCaretByCursor;
var
  ptCursor: TPoint;
  nChar: integer;
begin
...
procedure TISFEdit.WMChar(var Msg: TWMKey);
var
...
{$ENDIF}
{$IFDEF TMSDOTNET}
    msg.charcode := ord(FormatSettings.decimalseparator[1]);
{$ENDIF}
  end;
...
{$ENDIF}
{$IFDEF TMSDOTNET}
  and (msg.charcode <> vk_back) and (msg.charcode <> ord(FormatSettings.decimalseparator[1])) and not AllowMin(chr(msg.CharCode)) then Exit;
{$ENDIF}
  ...
  case EditType of
    ...
    etMoney:
      begin
        ...
        begin
{$IFNDEF TMSDOTNET}
          if (chr(msg.charcode) in [FormatSettings.ThousandSeparator, FormatSettings.decimalseparator]) then
{$ENDIF}
{$IFDEF TMSDOTNET}
...
{$ENDIF}
{$IFDEF TMSDOTNET}
            if scanprecision(self.Text, SelStart) and ((msg.charcode in [$30..$39]) or (msg.charcode = ord(FormatSettings.decimalseparator[1]))) and (SelLength = 0) then
{$ENDIF}
            begin
...
{$ENDIF}
{$IFDEF TMSDOTNET}
            if (SelStart = 0) and (self.Text = '') and (msg.charcode = ord(FormatSettings.decimalseparator[1])) then
{$ENDIF}
            begin
...
procedure TISFEdit.CreateParams(var Params: TCreateParams);
begin
...
procedure TISFEdit.CMFontChanged(var Message: TMessage);
begin
...
procedure TISFEdit.CMCancelMode(var Message: TMessage);
begin
  inherited;
...
procedure TISFEdit.SetFlatRect(const Value: Boolean);
var
  loc: TRect;
  lft: integer;
begin
...
procedure TISFEdit.SetFlat(const value: boolean);
var
  OldColor: TColor;
...
procedure TISFEdit.CNCtlColorEdit(var Message: TWMCtlColorEdit);
begin
  inherited;
...
procedure TISFEdit.CNCtlColorStatic(var Message: TWMCtlColorStatic);
begin
  inherited;
  if FTransparent then
    SetBkMode(Message.ChildDC, Windows.TRANSPARENT);
...
procedure TISFEdit.SetTransparent(const Value: boolean);
begin
...
procedure TISFEdit.SetFlatLineColor(const Value: TColor);
begin
  FFlatLineColor := Value;
  Invalidate;
end;

procedure TISFEdit.SetFlatParentColor(const Value: boolean);
begin
  FFlatParentColor := Value;
  Invalidate;
end;

procedure TISFEdit.SetEditType(value: TAdvEditType);
var
  at: TAutoType;
begin
...
procedure TISFEdit.SetEditAlign(const Value: TEditAlign);
begin
...
procedure TISFEdit.SetCanUndo(const Value: boolean);
begin
...
procedure TISFEdit.WMActivate(var Message: TMessage);
begin
  inherited;
end;

procedure TISFEdit.WMKillFocus(var Msg: TWMKillFocus);
var
  IsValid: Boolean;
  OldModified: Boolean;
  TT: string;
...
procedure TISFEdit.SelectBeforeDecimal;
var
  i: Integer;
begin
...
procedure TISFEdit.SelectAfterDecimal;
var
  i: Integer;
begin
  i := Pos(FormatSettings.decimalseparator, self.Text);
...
procedure TISFEdit.WMSetFocus(var Msg: TWMSetFocus);
begin
  if csLoading in ComponentState then
    Exit;
...
constructor TISFEdit.Create(AOwner: TComponent);
var
...
procedure TISFEdit.SetParent(AParent: TWinControl);
begin
  inherited;
  if FLabel <> nil then
...
procedure TISFEdit.ApplyURL(const Value: Boolean);
begin
  if Value then
  begin
...
procedure TISFEdit.CNCommand(var Message: TWMCommand);
begin
...
procedure TISFEdit.WMKeyDown(var Msg: TWMKeydown);
var
  selp: Integer;
  s: string;
  isCtrl: Boolean;
...
procedure TISFEdit.WMCopy(var Message: TWMCopy);
var
  Allow: Boolean;
begin
  Allow := True;
...
procedure TISFEdit.WMCut(var Message: TWMCut);
var
  Allow: Boolean;
begin
  Allow := True;
...
procedure TISFEdit.WMPaste(var Msg: TMessage);
var
  Data: THandle;
{$IFNDEF TMSDOTNET}
  content: PChar;
...
procedure TISFEdit.WMPaint(var Msg: TWMPaint);
begin
  inherited;
  PaintEdit;
...
procedure TISFEdit.DrawErrorLines(Canvas: TCanvas; ErrPos, ErrLen: Integer);
var
  pt1: TPoint;
  pt2: TPoint;
  l: Integer;
...
procedure TISFEdit.WMEraseBkGnd(var Message: TWMEraseBkGnd);
var
  DC: HDC;
  i: Integer;
  p: TPoint;
...
procedure TISFEdit.WMLButtonDown(var Msg: TWMMouse);
var
  uchar: Integer;
  show: Boolean;
begin
...
procedure TISFEdit.WMLButtonUp(var Msg: TWMMouse);
var
  uchar: Integer;
...
procedure TISFEdit.SetPrefix(const Value: string);
var
  s: string;
begin
  s := self.Text;
...
procedure TISFEdit.SetSuffix(const Value: string);
var
  s: string;
begin
  s := self.text;
...
function TISFEdit.DecimalPos: Integer;
var
  i: Integer;
begin
  i := Pos(FormatSettings.decimalseparator, self.text);
...
function TISFEdit.AllowMin(ch: char): boolean;
begin
  Result := Signed and (EditType in [etFloat,etNumeric, etMoney]) and (ch = '-');
end;

function TISFEdit.FixedLength(s: string): Integer;
var
  i: Integer;
begin
  s := StripThousandSep(s);
...
function TISFEdit.GetText: string;
var
  s: string;
begin
  s := inherited Text;
...
procedure TISFEdit.SetText(value: string);
var
  fmt, neg: string;
  f: extended;
begin
...
procedure TISFEdit.SetTextDirect(s: string);
begin
  inherited Text := s;
end;

function TISFEdit.GetVisible: boolean;
begin
  Result := inherited Visible;
end;

procedure TISFEdit.CMShowingChanged(var Message: TMessage);
begin
  inherited;
  if (FLabel <> nil) then FLabel.Visible := Visible;
end;

procedure TISFEdit.SetVisible(const Value: boolean);
begin
...
function TISFEdit.CreateLabel: TLabel;
begin
...
function TISFEdit.GetLabelCaption: string;
begin
  if FLabel <> nil then
...
{$IFNDEF DELPHI3_LVL}
function TISFEdit.GetSelText: string;
var
...
procedure TISFEdit.AutoSeparators;
var
  s, si, neg: string;
  d: Double;
...
procedure TISFEdit.UpdateLabel;
begin
...
procedure TISFEdit.SetLabelPosition(const value: TLabelPosition);
begin
...
procedure TISFEdit.SetLabelMargin(const value: integer);
begin
  FLabelMargin := Value;
  if FLabel <> nil then UpdateLabel;
end;

procedure TISFEdit.SetLabelFont(const Value: TFont);
begin
...
procedure TISFEdit.LabelFontChange(Sender: TObject);
begin
  if FLabel <> nil then
    UpdateLabel;
end;

procedure TISFEdit.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
...
procedure TISFEdit.SetLabelCaption(const value: string);
begin
  if FLabel = nil then
    FLabel := CreateLabel;
...
procedure TISFEdit.SetLabelTransparent(const value: boolean);
begin
  FLabelTransparent := Value;
  if FLabel <> nil then UpdateLabel;
end;

procedure TISFEdit.CMMouseEnter(var Msg: TMessage);
begin
  inherited;
...
procedure TISFEdit.CMMouseLeave(var Message: TMessage);
begin
  inherited;
...
function TISFEdit.GetFloat: double;
var
  s: string;
  d: double;
  e: integer;
...
function TISFEdit.GetInt: integer;
begin
  Result := 0;
  case FEditType of
    etHex: if (self.Text <> '') then Result := HexToInt(self.Text);
...
procedure TISFEdit.SetFloat(const Value: double);
var
  s:string;
begin
  case FEditType of
...
procedure TISFEdit.SetInt(const Value: integer);
begin
  case FEditType of
...
procedure TISFEdit.SetPrecision(const Value: smallint);
var
  at: TAutoType;
begin
...
procedure TISFEdit.SetPrecisionDisplay(const Value: TPrecisionDisplay);
begin
...
function TISFEdit.GetModified: boolean;
begin
  Result := fIsModified;
end;

procedure TISFEdit.SetModified(const Value: boolean);
begin
  if csLoading in ComponentState then
    Exit;
...
procedure TISFEdit.ListToRangeStr(rangelist: TRangeList);
var
  c: Integer;
...
function TISFEdit.RangeStrToList(rangelist: TRangeList): Boolean;
begin
  Result := RangeList.StrToList(self.Text);
end;

procedure TISFEdit.LoadPersist;
var
  Inifile: TInifile;
{$IFDEF DELPHI3_LVL}
...
procedure TISFEdit.SavePersist;
var
  Inifile: TInifile;
{$IFDEF DELPHI3_LVL}
  RegInifile: TRegInifile;
...
procedure TISFEdit.WMDestroy(var Msg: TMessage);
begin
  if not (csDesigning in ComponentState) then
    SavePersist;
  DefaultHandler(msg);
...
function TISFEdit.TestURL: Boolean;
begin
  Result := (Pos('://', self.text) > 0) or (Pos('@', self.text) > 1) or (Pos('www.', lowercase(self.Text)) = 1);
end;

function TISFEdit.GetError: Boolean;
begin
  Result := FError;
end;

procedure TISFEdit.SetError(const Value: Boolean);
begin
...
procedure TISFEdit.DrawControlBorder(DC: HDC);
var
  ARect: TRect;
...
function TISFEdit.Is3DBorderButton: Boolean;
begin
  if csDesigning in ComponentState then
    Result := Enabled
  else
...
{$IFNDEF TMSDOTNET}
procedure TISFEdit.WMNCPaint(var Message: TMessage);
begin
  inherited;
...
{$IFDEF TMSDOTNET}
procedure TISFEdit.WndProc(var Message: TMessage);
begin
  inherited;
end;
{$ENDIF}

function TISFEdit.GetEnabledEx: boolean;
begin
  Result := inherited Enabled;
end;

procedure TISFEdit.SetEnabledEx(const Value: boolean);
var
  OldValue: Boolean;
  OldColor: TColor;
begin
...
procedure TISFEdit.SetDisabledColor(const Value: TColor);
begin
  FDisabledColor := Value;
  Invalidate;
end;

procedure TISFEdit.SetDisabledBorder(const Value: boolean);
begin
...
function TISFEdit.GetColorEx: TColor;
begin
  Result := inherited Color;
end;

procedure TISFEdit.SetColorEx(const Value: TColor);
begin
...
{$IFNDEF TMSDOTNET}
procedure TISFEdit.CMParentFontChanged(var Message: TMessage);
begin
  inherited;
...
{$IFNDEF TMSDOTNET}
procedure TISFEdit.CMHintShow(var Msg: TMessage);
{$IFNDEF DELPHI3_LVL}
type
  PHintInfo = ^THintInfo;
{$ENDIF}
{$ENDIF}
{$IFDEF TMSDOTNET}
  procedure TISFEdit.CMHintShow(var Message: TCMHintShow);
  {$ENDIF}
  var
  {$IFNDEF TMSDOTNET}
    hi: PHintInfo;
...
procedure TISFEdit.SetAutoThousandSeparator(const Value: Boolean);
begin
  FAutoThousandSeparator := Value;
  if FEditType in [etMoney, etFloat] then AutoSeparators;
end;

procedure TISFEdit.SetEmptyText(const Value: string);
begin
  FEmptyText := Value;
  Invalidate;
end;

procedure TISFEdit.CMWantSpecialKey(var Msg: TCMWantSpecialKey);
var
  IsPrev: Boolean;
  p: TWinControl;
...
procedure TISFEdit.SetSoftBorder(const Value: Boolean);
begin
...
procedure TISFEdit.SetLabelAlwaysEnabled(const Value: Boolean);
begin
...
procedure TISFEdit.SetBorder3D(const Value: Boolean);
begin
  FBorder3D := Value;
  Invalidate;
end;

procedure TISFEdit.CreateWnd;
begin
  inherited;
...
procedure TISFEdit.SetErrorMarkerLen(const Value: Integer);
begin
  FErrorMarkerLen := Value;
  Invalidate;
end;

procedure TISFEdit.SetErrorMarkerPos(const Value: Integer);
begin
  FErrorMarkerPos := Value;
  Invalidate;
end;

procedure TISFEdit.SetFocusBorder(const Value: Boolean);
begin
  FFocusBorder := Value;
  Invalidate;
end;

function TISFEdit.GetHeightEx: Integer;
begin
  Result := inherited Height;
end;

procedure TISFEdit.SetHeightEx(const Value: Integer);
begin
  if (csLoading in ComponentState) then
    FLoadedHeight := Value;
...
procedure TISFEdit.ListMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
  DoneLookup;
end;

procedure TISFEdit.DoneLookup;
var
  idx, vp: Integer;
  NewValue: string;
...
procedure TISFEdit.ListKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    DoneLookup;
end;

procedure TISFEdit.UpdateLookup;
var
  pt, cp: TPoint;
  i, cnt, tp: Integer;
  mw, tw, vp: Integer;
...
procedure TISFEdit.LabelClick(Sender: TObject);
begin
  if Assigned(FOnLabelClick) then
    FOnLabelClick(Self);
end;

procedure TISFEdit.LabelDblClick(Sender: TObject);
begin
  if Assigned(FOnLabelDblClick) then
    FOnLabelDblClick(Self);
end;

function TISFEdit.DoValidate(value: string): Boolean;
var
  IsValid: Boolean;
begin
  IsValid := True;
...
procedure TISFEdit.ValidateEvent(Value: string; var IsValid: Boolean);
begin
  if Assigned(FOnValueValidate) then
    FOnValueValidate(Self, value, IsValid);
end;

function TISFEdit.GetVersion: string;
var
  vn: Integer;
begin
  vn := GetVersionNr;
...
function TISFEdit.GetVersionNr: Integer;
begin
  Result := MakeLong(MakeWord(BLD_VER, REL_VER), MakeWord(MIN_VER, MAJ_VER));
end;

procedure TISFEdit.SetVersion(const Value: string);
begin
...
constructor TISFMaskEdit.Create(AOwner: TComponent);
var
  i: integer;
begin
  inherited Create(aOwner);
...
procedure TISFMaskEdit.SetAlignment(value: tAlignment);
begin
...
procedure TISFMaskEdit.CreateParams(var Params: TCreateParams);
begin
  inherited CreateParams(params);
...
procedure TISFMaskEdit.KeyUp(var Key: Word; Shift: TShiftState);
var
  accept: Boolean;
...
procedure TISFMaskEdit.DoEnter;
begin
...
procedure TISFMaskEdit.CMMouseEnter(var Msg: TMessage);
begin
  if FAutoFocus then
    self.SetFocus;
...
procedure TISFMaskEdit.CMMouseLeave(var Msg: TMessage);
begin
  inherited;
  if FMouseInControl and Enabled then
  begin
...
procedure TISFMaskEdit.WMKillFocus(var Msg: TWMKillFocus);
begin
  inherited;
...
procedure TISFMaskEdit.WMSetFocus(var Msg: TWMSetFocus);
begin
...
procedure TISFMaskEdit.WMChar(var Msg: TWMKey);
begin
  if (msg.charcode = VK_RETURN) and (FReturnIsTab) then
    Exit;
  inherited;
...
function TISFMaskEdit.GetLabelCaption: string;
begin
  if FLabel <> nil then Result := FLabel.caption else Result := '';
end;

procedure TISFMaskEdit.SetLabelCaption(const value: string);
begin
...
function TISFMaskEdit.CreateLabel: TLabel;
begin
  Result := TLabel.Create(Self);
  Result.Parent := Self.Parent;
  Result.FocusControl := Self;
...
procedure TISFMaskEdit.SetLabelMargin(const Value: integer);
begin
  FLabelMargin := Value;
  if FLabel <> nil then UpdateLabel;
end;

procedure TISFMaskEdit.SetLabelTransparent(const Value: boolean);
begin
  FLabelTransparent := Value;
  if FLabel <> nil then UpdateLabel;
end;
...
procedure TISFMaskEdit.SetLabelPosition(const Value: TLabelPosition);
begin
  FLabelPosition := Value;
  if FLabel <> nil then UpdateLabel;
end;

procedure TISFMaskEdit.UpdateLabel;
begin
  FLabel.Transparent := FLabeltransparent;
  case FLabelPosition of
    lpLeftTop: begin
...
destructor TISFMaskEdit.Destroy;
begin
  if FLabel <> nil then
  begin
    FLabel.Parent := nil;
...
procedure TISFMaskEdit.SetBounds(ALeft, ATop, AWidth, AHeight: Integer);
begin
  inherited SetBounds(ALeft, ATop, AWidth, AHeight);
...
procedure TISFMaskEdit.SetFlat(const Value: boolean);
begin
  if FFlat <> Value then
  begin
    FFlat := Value;
...
procedure TISFMaskEdit.CMFontChanged(var Message: TMessage);
begin
  if (csDesigning in ComponentState) then
    if FLabel <> nil then FLabel.Font.Assign(self.font);
  inherited;
...
{$IFNDEF TMSDOTNET}
procedure TISFMaskEdit.WMNCPaint(var Message: TMessage);
begin
  inherited;
  if (FFocusBorderColor <> clNone) or not Enabled then
    DrawBorder;
...
procedure TISFMaskEdit.WMPaint(var Msg: TWMPaint);
begin
  inherited;
  PaintEdit;
  if Border3D or (FFocusBorderColor <> clNone) then
...
procedure TISFMaskEdit.WMKeyDown(var Msg: TWMKeydown);
begin
  if (msg.CharCode = VK_RETURN) and (FReturnIsTab) then
  begin
    msg.CharCode := VK_TAB;
...
function TISFMaskEdit.GetModified: boolean;
begin
  Result := inherited Modified;
end;

procedure TISFMaskEdit.SetModified(const Value: boolean);
begin
  if FShowModified then
  begin
    if Value = False then
...
procedure TISFMaskEdit.SetDisabledColor(const Value: TColor);
begin
  FDisabledColor := Value;
  Invalidate;
end;

procedure TISFMaskEdit.SetDisabledBorder(const Value: boolean);
begin
  FDisabledBorder := Value;
  Invalidate;
end;

function TISFMaskEdit.GetEnabledEx: Boolean;
begin
  Result := inherited Enabled;
end;

procedure TISFMaskEdit.SetEnabledEx(const Value: Boolean);
var
  OldValue: Boolean;
  OldColor: TColor;
begin
...
function TISFMaskEdit.GetColorEx: TColor;
begin
  Result := inherited Color;
end;

procedure TISFMaskEdit.SetColorEx(const Value: TColor);
begin
  if csLoading in ComponentState then
    FLoadedColor := Value;
...
procedure TISFMaskEdit.SetLabelFont(const Value: TFont);
begin
  FLabelFont.Assign(Value);
  if Assigned(FLabel) then
    FLabel.Font.Assign(FLabelFont);
...
procedure TISFMaskEdit.LabelFontChanged(Sender: TObject);
begin
  if Assigned(FLabel) then
    FLabel.Font.Assign(FLabelFont);
end;
...
function TISFMaskEdit.GetVisible: Boolean;
begin
  Result := inherited Visible;
end;

procedure TISFMaskEdit.SetVisible(const Value: Boolean);
begin
  inherited Visible := Value;
  if (FLabel <> nil) then
...
procedure TISFMaskEdit.DrawBorder;
var
  DC: HDC;
begin
  if Enabled and not (FFlat or (FFocusBorder and FMouseInControl) or Border3D or (FFocusBorderColor <> clNone)) then
...
procedure TISFMaskEdit.DrawControlBorder(DC: HDC);
var
  ARect: TRect;
  BtnFaceBrush, WindowBrush: HBRUSH;
begin
...
function TISFMaskEdit.Is3DBorderButton: Boolean;
begin
  if csDesigning in ComponentState then
    Result := Enabled
  else
...
procedure TISFMaskEdit.SetFlatLineColor(const Value: TColor);
begin
  FFlatLineColor := Value;
  Invalidate;
end;

procedure TISFMaskEdit.PaintEdit;
var
  DC: HDC;
  Oldpen: HPen;
  Loc: TRect;
...
procedure TISFMaskEdit.SetFlatRect(const Value: Boolean);
var
  loc: TRect;
begin
  if Value then
...
procedure TISFMaskEdit.SetSoftBorder(const Value: Boolean);
begin
  FSoftBorder := Value;
  Invalidate;
end;

procedure TISFMaskEdit.SetBorder3D(const Value: Boolean);
begin
  FBorder3D := Value;
end;

procedure TISFMaskEdit.SetFlatParentColor(const Value: Boolean);
begin
  FFlatParentColor := Value;
  Invalidate;
end;

procedure TISFMaskEdit.CreateWnd;
begin
  inherited;
  SetFlatRect(FFlat);
end;

function TISFMaskEdit.GetVersion: string;
var
  vn: Integer;
begin
  vn := GetVersionNr;
...
function TISFMaskEdit.GetVersionNr: Integer;
begin
  Result := MakeLong(MakeWord(BLD_VER, REL_VER), MakeWord(MIN_VER, MAJ_VER));
end;

procedure TISFMaskEdit.SetVersion(const Value: string);
begin

end;

procedure TISFMaskEdit.SetAutoFocus(const Value: boolean);
begin
  FAutoFocus := Value;
end;
