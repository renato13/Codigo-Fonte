...
unit FGeraParcelasCupomNaoFiscal;
...
type
   TFrmGeraParcelasCupomNaoFiscal = class(TForm)
...
var
   FrmGeraParcelasCupomNaoFiscal: TFrmGeraParcelasCupomNaoFiscal;
   i, TotReg: integer;
   nContador, xx_prazos,
   nCodigoCHR: integer;
...
uses FPrincipal, SQLServer, Biblioteca, Global, FVendaExpressa,
  FBaseDados, Classe.Usuarios;

{$R *.DFM}

procedure TFrmGeraParcelasCupomNaoFiscal.FormCreate(Sender: TObject);
var
 vnCreditoSaldo : Real;
begin
   ...
   FrmVendaExpressa.db_vendas.Edit;
   FrmVendaExpressa.db_VendasItens.Edit;

end;

procedure TFrmGeraParcelasCupomNaoFiscal.FormClose(Sender: TObject;
...
procedure TFrmGeraParcelasCupomNaoFiscal.bbtn_excluiClick(Sender: TObject);
begin
   db_CtaReceber.delete;
end;

procedure TFrmGeraParcelasCupomNaoFiscal.BtnGerarClick(Sender: TObject);
begin

   {
   wTipoPagamento := '';

   if (FrmVendaExpressa.db_vendas.Fieldbyname('CLIENTE').AsInteger = 0) or
     (FrmVendaExpressa.db_vendas.Fieldbyname('CLIENTE').IsNull) then
   begin
      BeepCritica;
   ...
   if (FrmVendaExpressa.db_vendas.Fieldbyname('FORMAPAGTO').AsInteger = 0) or
     (FrmVendaExpressa.db_vendas.Fieldbyname('FORMAPAGTO').IsNull) then
   begin
      BeepCritica;
   ...
   db_FormaPagto.ParamByName('CODIGO').AsInteger :=
     FrmVendaExpressa.db_vendas.Fieldbyname('FORMAPAGTO').AsInteger;
   db_FormaPagto.open;
   ...
   If (FrmVendaExpressa.db_vendas.state in [dsEdit, dsInsert]) then
      FrmVendaExpressa.db_vendas.Post;
   ...
   db_CtaReceber.open;
   db_FormaPagto.close;
   db_FormaPagto.ParamByName('FORMAPAGTO').AsInteger := FrmVendaExpressa.db_vendas.FieldByName('FORMAPAGTO').AsInteger;
   db_FormaPagto.open;
   ...
   // cofigurar
   forma := FrmVendaExpressa.db_vendas.FieldByName('FORMAPAGTO').AsString;
   ...
   tt_nota := tt_nota - FrmVendaExpressa.db_vendas.FieldByName('VLR_RECEBIDO').asFloat;

   tt_nota      := tt_nota - FrmVendaExpressa.db_vendas.FieldByName('CREDITOABATIMENTO').asFloat ; // abatimento de credito
...
procedure TFrmGeraParcelasCupomNaoFiscal.CalcGrade;
begin
   ...
   begin
      ...
      db_CtaReceber.FieldByName('DATA').asDateTime :=                FrmVendaExpressa.db_vendas.FieldByName('EMISSAO').asDateTime;
      ...
      db_CtaReceber.FieldByName('FORMA_PAGTO').AsString :=           FrmVendaExpressa.db_vendas.FieldByName('FORMAPAGTO').AsString;
      db_CtaReceber.FieldByName('VENDEDOR').AsString :=              FrmVendaExpressa.db_vendas.FieldByName('VENDEDOR').AsString;
      db_CtaReceber.FieldByName('CLIENTE').AsInteger :=              FrmVendaExpressa.db_vendas.FieldByName('CLIENTE').AsInteger;
      db_CtaReceber.FieldByName('NOTAFISCAL').AsInteger :=           FrmVendaExpressa.db_vendas.FieldByName('CODIGO').AsInteger;
      ...
      if bGerarParcelaExterna=false then
         db_CtaReceber.FieldByName('DOCUMENTO').AsString :=             FrmVendaExpressa.db_vendas.FieldByName('NOTAFISCAL').AsString + '/' +      ZeroEsquerda(IntToStr(parc_atu),3)
      else
      begin
         inc(nCodigoCHR);
         db_CtaReceber.FieldByName('DOCUMENTO').AsString :=             FrmVendaExpressa.db_vendas.FieldByName('NOTAFISCAL').AsString + '/' +      ZeroEsquerda(IntToStr(parc_atu),3) + Chr(nCodigoCHR);
      end;
...
procedure TFrmGeraParcelasCupomNaoFiscal.BtnSairClick(Sender: TObject);
begin
...
procedure TFrmGeraParcelasCupomNaoFiscal.DBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
...
procedure TFrmGeraParcelasCupomNaoFiscal.BtnExcluirClick(Sender: TObject);
begin
   ...
   EditRecebido.enabled := True;

end;

procedure TFrmGeraParcelasCupomNaoFiscal.FormKeyDown(Sender: TObject;
...
procedure TFrmGeraParcelasCupomNaoFiscal.db_CtaReceberAfterPost
  (DataSet: TDataSet);
begin
...
procedure TFrmGeraParcelasCupomNaoFiscal.Panel2Exit(Sender: TObject);
begin
   If (FrmVendaExpressa.db_vendas.state in [dsEdit, dsInsert]) then
      FrmVendaExpressa.db_vendas.Post;
...
procedure TFrmGeraParcelasCupomNaoFiscal.BtnOKClick(Sender: TObject);
begin
   {
     if db_CtaReceber.IsEmpty then
     begin
...
procedure TFrmGeraParcelasCupomNaoFiscal.db_CtaReceberNewRecord
  (DataSet: TDataSet);
begin
   EditRecebido.enabled := false;
