...
uses
  ...
  FireDAC.Comp.Client, Vcl.Samples.Spin, ISFEdit, ISFEditbtn, ISFdbEditbtn,
...
begin
   ...
   IMGLogoRelatorios.Picture.LoadFromFile(db_Parametros.FieldByName('SISTEMA_LOGO_RELATORIOS').AsString);
...
procedure TFrmParametrosSistema.PreecherSpinEdit;
begin
...
procedure TFrmParametrosSistema.SalvarSpinEdit;
begin
   if FCarregando then
     exit;
...
begin
   ...
   Try
      IMGLogomarca.Picture.LoadFromFile(db_Parametros.FieldByName('SISTEMA_LOGO_PRINCIPAL').AsString);
   Except
   End;
